...
from tools.utility import Singleton, StoppableThread, timestamp, ClosingHandler

from traits.api import HasTraits, Instance, Enum, Range, Button, Bool
...
class Job( HasTraits ):
    ...
    min_view = View(HGroup(HGroup(Item('edit', show_label=False),
...
class ManagedJob( Job ):
    ...
    submit_button = Button(label='Start', desc='Submit the measurement to the job manager.')
    remove_button = Button(label='Stop', desc='Remove the measurement from the job manager. Stop it if necessary.')
