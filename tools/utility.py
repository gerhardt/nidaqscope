...
class ClosingHandler( Handler ):
    """Make a closing function for view aveilable.
    
    In order to close a view, create a close trait. The view will close when
    this trait changes.
    """
    def object_close_changed(self, info):
        info.ui.dispose()

class GetSetItemsHandler( ClosingHandler ):
...
# Matplotlib figures in traits
# http://code.enthought.com/projects/traits/docs/html/tutorials/traits_ui_scientific_app.html
# (usefull when chaco meets its limitations and speed is not an issue)

import wx
import matplotlib
# matplotlib should use a wxPython backend
...
class _MPLFigureEditor(Editor):
    ...
    def update_editor(self):
        # update by clearing axes and calling users mpl.figure.canvas.draw()
        pass
