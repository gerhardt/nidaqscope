...
class CounterBoard:
    ...
    def ReadAI(self):
        CHK( dll.DAQmxReadAnalogF64(self.AITask,
                                    ctypes.c_long(self.N),
                                    ctypes.c_double(self.read_timeout),
                                    DAQmx_Val_GroupByChannel,
                                    self._AIData.ctypes.data_as(c_float64_p),
    ...
    def WaitAI(self):
        CHK( dll.DAQmxWaitUntilTaskDone(self.AITask, ctypes.c_double(self._TaskTimeout))  )
...
class Scanner( MultiBoard ):
    ...
    def scanAnalogLine(self, Line, SecondsPerPoint, return_speed=None):
        """Perform a line scan. If return_speed is not None, return to beginning of line
        with a speed 'return_speed' times faster than the speed currently set.
        """
        self.setTiming(SecondsPerPoint*0.1, SecondsPerPoint*0.9)
    ...
    def PosToVolt(self, r):
        ...
        dv2 = v2[1]-v2[0]
        if self.invert_x:
            vx = v10+(x[1]-r[0])/(x[1]-x[0])*dv1           
        else:
            vx = v10+(r[0]-x[0])/(x[1]-x[0])*dv1            
        if self.invert_y:
            vy = v10+(y[1]-r[1])/(y[1]-y[0])*dv1            
        else:
            vy = v10+(r[1]-y[0])/(y[1]-y[0])*dv1            
        if self.invert_z:
            vz = v20+(z[1]-r[2])/(z[1]-z[0])*dv2            
        else:
            vz = v20+(r[2]-z[0])/(z[1]-z[0])*dv2
        if self.swap_xy:
            vt = vx
            vx = vy
            vy = vt            
...
class AnalogInOut():
    ...
    def __init__(self, AOChan, TickSource, AIChan, ao_range=(-10,10), ai_range=(-10,10), duty_cycle=0.96):
        ao_task = ctypes.c_ulong() # analog out
        co_task = ctypes.c_ulong() # finite pulse train
        ai_task = ctypes.c_ulong() # finite pulse train
        ...
        CHK( dll.DAQmxSetCICountEdgesTerm(ai_task, ai_dev, co_dev+'InternalOutput') )
        CHK( dll.DAQmxSetCICtrTimebaseSrc(ai_task, ai_dev, ai_port) )

        # read samples from beginning of acquisition, do not overwrite
        CHK( dll.DAQmxSetReadRelativeTo(ai_task, DAQmx_Val_CurrReadPos) )
        CHK( dll.DAQmxSetReadOffset(ai_task, 0) )
        CHK( dll.DAQmxSetReadOverWrite(ai_task, DAQmx_Val_DoNotOverwriteUnreadSamps) )
    ...
    def configure(self, n_samples, seconds_per_point):
        ...
        if n_samples == 0:
            ...
            CHK( dll.DAQmxCfgImplicitTiming(self.ai_task,
                                            DAQmx_Val_FiniteSamps,
                                            ctypes.c_ulonglong(n_samples+1)
                                            ))
            self.ai_data = numpy.empty((n_samples+1,), dtype=numpy.double)
    ...
    def line(self, voltage, seconds_per_point):
        ...
        CHK( dll.DAQmxReadAnalogF64(ai_task,
                                     ctypes.c_int32(n+1),
                                     ctypes.c_double(1.0),
        ...
        CHK( dll.DAQmxStopTask(ai_task) )
        CHK( dll.DAQmxStopTask(co_task) )
        
        return ai_data[:-1] / ( self.seconds_per_point * self.duty_cycle ) # -ci_data[:-1]
